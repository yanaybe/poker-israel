...
const VIBE_LABELS: Record<string, string> = {
  SERIOUS: '🎯 רציני', FUN_SOCIAL: '🎉 כיפי/חברתי',
  BEGINNERS_WELCOME: '🌱 מתחילים מוזמנים', REGULARS_ONLY: '🔒 רגולרים בלבד', QUIET: '🤫 משחק שקט',
}
const PACE_LABELS: Record<string, string> = { FAST: '⚡ מהיר', NORMAL: '⏱ רגיל', SLOW: '🐢 איטי/לייזר' }
const REBUY_LABELS: Record<string, string> = { ALLOWED: '✅ מותרים', CAPPED: '🔢 מוגבלים', NONE: '❌ אסורים' }
...
export default function GameDetailPage() {
  ...
  useEffect(() => {
    if (!game) return
    const needsSimilar = ['FULL', 'CLOSED', 'CANCELLED'].includes(game.status)
    if (!needsSimilar) return
    fetch(`/api/games?city=${encodeURIComponent(game.city)}&gameType=${game.gameType}&status=OPEN`)
      .then((r) => r.json())
        ...
        if (filtered.length < 3) {
          ...
          fetch(`/api/games?city=${encodeURIComponent(game.city)}&status=OPEN`)
            ...
            .then((all: GameWithHost[]) => {
              ...
              all.filter((g) => g.id !== game.id && !merged.find((x) => x.id === g.id))
                .slice(0, 3 - merged.length)
                .forEach((g) => merged.push(g))
              setSimilarGames(merged)
            })
        } else {
  ...
  const handleCancelGame = async () => {
    setCancelling(true)
    try {
      await fetch(`/api/games/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: 'CANCELLED' }) })
      setCancelModal(false); fetchGame()
  ...
  const isApprovedPlayer = myRequest?.status === 'APPROVED'
  let locationDisplay: { text: string; icon: React.ReactNode; sub?: string }
  if (game.locationRevealed && game.location) {
    locationDisplay = {
      text: game.location,
      icon: <Unlock className="w-4 h-4 text-green-400" />,
      sub: game.neighborhood ? `${game.neighborhood}, ${game.city}` : game.city,
    }
  } else if (game.neighborhood) {
    if (isApprovedPlayer) {
      const h = Math.ceil(hoursUntilGame)
      locationDisplay = {
        text: publicLocation,
        icon: <Lock className="w-4 h-4 text-gold-400" />,
        sub: `✅ אושרת! הכתובת תחשף ${h <= 2 ? 'בקרוב' : `בעוד ~${h - 2} שעות`}`,
      }
    } else {
      locationDisplay = {
        text: publicLocation,
        icon: <Lock className="w-4 h-4 text-poker-subtle" />,
        sub: '🔒 כתובת מדויקת תחשף למאושרים 2 שעות לפני',
      }
    }
  } else {
    locationDisplay = { text: publicLocation, icon: <MapPin className="w-4 h-4" /> }
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex items-start justify-between gap-4 mb-4">
              ...
              {isHost && !isCancelled && (
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setCancelModal(true)} className="p-2 text-poker-muted hover:text-red-400 rounded-xl hover:bg-red-400/10 transition-all" title="בטל משחק">
                    <X className="w-4 h-4" />
                  </button>
                </div>
              )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
              ...
              <div className="flex items-start gap-3 p-3 bg-felt-900/50 rounded-xl sm:col-span-2">
                ...
                <div className="min-w-0">
                  <p className="text-xs text-poker-subtle">מיקום</p>
                  <p className="text-sm font-semibold text-poker-text">{locationDisplay.text}</p>
                  {locationDisplay.sub && <p className="text-xs text-poker-muted mt-0.5">{locationDisplay.sub}</p>}
                </div>
              </div>
              {[
            ...
            {!isCancelled && (
              <div className="mt-5">
                <div className="flex justify-between text-xs text-poker-subtle mb-2">
                  <span>מקומות תפוסים</span><span>{game.currentPlayers}/{game.maxPlayers}</span>
                </div>
                <div className="h-2 bg-felt-700 rounded-full overflow-hidden">
                  <div className={cn('h-full rounded-full transition-all', isFull ? 'bg-red-500' : 'bg-green-500')} style={{ width: `${fillPercent}%` }} />
                </div>
                {waitlistRequests.length > 0 && <p className="text-xs text-poker-subtle mt-1.5">{waitlistRequests.length} ממתינים ברשימת ההמתנה</p>}
              </div>
            )}
          </div>
          ...
          {(game.stackMin || game.gamePace || game.rebuyType || game.vibeTags || game.expectedDuration || game.hasFood || game.hasDrinks) && (
            <div className="glass-card rounded-2xl p-6 border border-felt-700/50">
              <h3 className="text-base font-bold text-poker-text mb-4">🔍 פרטי המשחק</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {(game.stackMin || game.stackMax) && (
                  <div className="flex items-start gap-2 p-3 bg-felt-900/50 rounded-xl">
                    <span className="text-lg">💰</span>
                    <div><p className="text-xs text-poker-subtle">סטקים</p>
                    <p className="text-sm font-semibold text-poker-text">{game.stackMin ? formatCurrency(game.stackMin) : '—'} – {game.stackMax ? formatCurrency(game.stackMax) : '—'}</p></div>
                  </div>
                )}
                {game.rebuyType && (
                  <div className="flex items-start gap-2 p-3 bg-felt-900/50 rounded-xl">
                    <span className="text-lg">🔄</span>
                    <div><p className="text-xs text-poker-subtle">ריי-בויים</p>
                    <p className="text-sm font-semibold text-poker-text">{REBUY_LABELS[game.rebuyType] ?? game.rebuyType}{game.rebuyType === 'CAPPED' && game.rebuyCap ? ` (מקס ${game.rebuyCap})` : ''}</p></div>
                  </div>
                )}
                {game.gamePace && (
                  <div className="flex items-start gap-2 p-3 bg-felt-900/50 rounded-xl">
                    <span className="text-lg">⏱</span>
                    <div><p className="text-xs text-poker-subtle">קצב</p>
                    <p className="text-sm font-semibold text-poker-text">{PACE_LABELS[game.gamePace] ?? game.gamePace}</p></div>
                  </div>
                )}
                {game.expectedDuration != null && game.expectedDuration > 0 && (
                  <div className="flex items-start gap-2 p-3 bg-felt-900/50 rounded-xl">
                    <span className="text-lg">🕐</span>
                    <div><p className="text-xs text-poker-subtle">משך משוער</p>
                    <p className="text-sm font-semibold text-poker-text">~{game.expectedDuration} שעות</p></div>
                  </div>
                )}
                {(game.hasFood || game.hasDrinks) && (
                  <div className="flex items-start gap-2 p-3 bg-felt-900/50 rounded-xl">
                    <span className="text-lg">{game.hasFood && game.hasDrinks ? '🍕🥤' : game.hasFood ? '🍕' : '🥤'}</span>
                    <div><p className="text-xs text-poker-subtle">כיבוד</p>
                    <p className="text-sm font-semibold text-poker-text">{[game.hasFood && 'אוכל', game.hasDrinks && 'שתייה'].filter(Boolean).join(' · ')}</p></div>
                  </div>
                )}
              </div>
              {vibeTags.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-2">
                  {vibeTags.map((tag) => (
                    <span key={tag} className="text-xs px-3 py-1 bg-felt-800/60 border border-felt-700/50 text-poker-muted rounded-full">{VIBE_LABELS[tag] ?? tag}</span>
                  ))}
                </div>
              )}
            </div>
          ...
          {isHost && game.requests && game.requests.length > 0 && (
            <div className="glass-card rounded-2xl p-6 border border-felt-700/50">
              <h3 className="text-lg font-bold text-poker-text mb-4">
                בקשות הצטרפות
                {pendingRequests.length > 0 && <span className="mr-2 px-2 py-0.5 bg-gold-500/20 text-gold-400 rounded-full text-sm">{pendingRequests.length} ממתינות</span>}
              </h3>
                ...
                {game.requests.map((req) => (
                  <div key={req.id} className="flex items-center justify-between gap-3 p-3 bg-felt-900/50 rounded-xl border border-felt-700/30">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      {req.status === 'PENDING' ? (
                        <>
                          <button onClick={() => handleRequestAction(req.id, 'APPROVED')} className="p-1.5 bg-green-500/20 text-green-400 hover:bg-green-500/30 rounded-lg transition-all" title="אשר"><Check className="w-4 h-4" /></button>
                          <button onClick={() => handleRequestAction(req.id, 'REJECTED')} className="p-1.5 bg-red-500/20 text-red-400 hover:bg-red-500/30 rounded-lg transition-all" title="דחה"><X className="w-4 h-4" /></button>
                        </>
                      ) : (
          ...
          {(['FULL', 'CLOSED', 'CANCELLED'] as const).includes(game.status as 'FULL' | 'CLOSED' | 'CANCELLED') && similarGames.length > 0 && (
            <div>
              <h2 className="text-lg font-bold text-poker-text mb-4 flex items-center gap-2">
                🃏 משחקים דומים השבוע
                <span className="text-sm font-normal text-poker-muted">הנה {similarGames.length} אופציות פתוחות</span>
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {similarGames.map((g) => <GameCard key={g.id} game={g} compact />)}
              </div>
            </div>
          )}
        </div>
          ...
          {!isHost && !isCancelled && (
            <div className="glass-card rounded-2xl p-5 border border-felt-700/50 space-y-3">
              {myRequest ? (
                ...
                <Button fullWidth variant={isFull ? 'outline' : 'gold'} disabled={!session} onClick={() => session ? setJoinModal(true) : router.push('/login')}>
                  {!session ? 'התחבר כדי להצטרף' : isFull ? 'הצטרף לרשימת ההמתנה' : 'בקש להצטרף'}
                </Button>
              )}
              <Button variant="outline" fullWidth onClick={() => session ? router.push(`/messages/${game.host.id}`) : router.push('/login')} className="gap-2">
                <MessageCircle className="w-4 h-4" />שלח הודעה למארח
              </Button>
            </div>
          )}
      ...
      <Modal isOpen={joinModal} onClose={() => setJoinModal(false)} title={isFull ? 'הצטרף לרשימת ההמתנה' : 'בקשת הצטרפות למשחק'}>
        <div className="space-y-4">
          {isFull && <div className="p-3 bg-gold-500/10 border border-gold-500/30 rounded-xl text-gold-400 text-sm">המשחק מלא. אם יתפנה מקום תקבל התראה ובקשתך תעבור לממתין.</div>}
        ...
        <div className="space-y-4">
          {hoursUntilGame < 3 && (
            <div className="flex items-start gap-3 p-3 bg-red-500/10 border border-red-500/30 rounded-xl">
              ...
              <p className="text-red-400 text-sm">המשחק מתחיל בפחות מ-3 שעות. ביטול כעת יסב לך <strong>סטרייק</strong>. 3 סטריקים ב-60 יום יחסמו אותך למשך 30 יום.</p>
            </div>
          )}
          <p className="text-poker-muted text-sm">כל השחקנים המאושרים יקבלו הודעה. פעולה זו אינה הפיכה.</p>
          <div className="flex gap-3">
            <Button variant="outline" fullWidth onClick={() => setCancelModal(false)}>חזור</Button>
            <Button fullWidth loading={cancelling} onClick={handleCancelGame} className="bg-red-500/20 text-red-400 border border-red-500/30 hover:bg-red-500/30">בטל משחק</Button>
          </div>
        </div>
      </Modal>
